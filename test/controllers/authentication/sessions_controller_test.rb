...
class Authentication::SessionsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should login an user by email" do
    post sessions_url, params: { login: @user.email, password: 'secure' } 
    assert_redirected_to products_url
  end

  test "should login an user by username" do
    post sessions_url, params: { login: @user.username, password: 'secure' } 
    assert_redirected_to products_url
  end
  ...
  test "should not logout" do
    ...
    assert_redirected_to products_url
    assert_equal flash[:notice], "Has cerrado sesión correctamente"
  end
